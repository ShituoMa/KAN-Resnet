...
from components.evaluations import train_and_test_models
...
def train_model_generic(model, train_ds, test_ds,device,epochs= 15,path =  "drive/MyDrive/KANs/models"):
    ...
    gen = torch.Generator()
    gen.manual_seed(0)
    mnist_train, mnist_val = torch.utils.data.random_split(train_ds, [int(0.8*len(train_ds)),len(train_ds)-int(0.8*len(train_ds))],
    generator=gen)
    # DataLoader
